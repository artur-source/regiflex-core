...
const QRCodeComponent = () => {
  ...
  const fetchPacientes = async () => {
    try {
      ...
      setError('ERR-FRONT-001: Erro ao carregar pacientes. Verifique a conexão com a API.');
    }
  };
  ...
  const handleGenerateQR = async (paciente) => {
    ...
    try {
      ...
      setError(error.message || 'ERR-FRONT-003: Erro ao gerar QR Code. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleScanQR = async () => {
    if (!scannedData.trim()) {
      setError('ERR-FRONT-003: Digite os dados do QR Code para leitura.');
      return;
    }
    ...
    try {
      ...
      setError(error.message || 'ERR-FRONT-003: QR Code inválido. Os dados não correspondem a um paciente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileUpload = (event) => {
    ...
    setError('Funcionalidade de upload de imagem (ERR-FRONT-003) será implementada em versões futuras.');
  };
